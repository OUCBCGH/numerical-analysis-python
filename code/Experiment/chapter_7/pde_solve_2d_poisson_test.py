...
class PDESolvePoisson2dModel:
    ...
    def fit_pde(self):
        ...
        # 构造右端向量，构成右端fi
        fi = PoissonModel.fun_xy(xm[1: -1, 1: -1], ym[1: -1, 1: -1])  # pde右端方程，内部节点(n-1)*(m-1)
        ...
        if self.is_show:
            self.plt_2d_poisson(xm, ym, u_xy, self.is_exact_fun)  # 可视化泊松方程解的图像
        return xm, ym, u_xy
    ...
    def _solve_sparse_matrix_method_(sp_mat, b):
        ...
        sdm = SteepestDescentMethod(sp_mat, b, x0, 1e-5, max_iter=400, is_out_info=True)  # 最速下降法
        sdm.fit_solve()
        sdm_iter = sdm.iterative_info["Iteration_number"]
        gs = JacobiGSlIterativeMethod(sp_mat, b, x0, eps=1e-5, max_iter=400, method="g-s", is_out_info=True)  # G-S迭代法
        gs.fit_solve()
        gs_iter = gs.iterative_info["Iteration_number"]

        # 如下代码可实现三种方法的可视化
        plt.figure(figsize=(7, 5))
        plt.semilogy(range(1, pcg_iter + 1), pcg.precision, "o-", lw=2, label="$PCG, \ k=%d$" % pcg_iter)
        plt.semilogy(range(1, cg_iter + 1), cg.precision, "s--", lw=2, label="$CG, \ k=%d$" % cg_iter)
        plt.semilogy(range(1, sdm_iter + 1), sdm.precision, "+-.", lw=2, label="$SD, \ k=%d$" % sdm_iter)
        plt.semilogy(range(1, gs_iter + 1), gs.precision, "*:", lw=2, label="$GS, \ k=%d$" % gs_iter)

...
equation = lambda x: np.exp(x) + 10 * x - 2  # 待求方程
...
for i, funEval in enumerate(funEvals):
    ism = IntervalSegmentation_Root(equation, span, eps=1e-16, display="display", funEval=funEval)  # 实例化对象
    ism.fit_root()  # 求近似根
...
plt.subplot(121)
plt.plot(np.arange(1, len(roots[0]) + 1), roots[0], "-s", lw=2,
         label="$%s: \ x^*=%.15f$" % (funEvals[0], roots[0][-1]))
...
plt.title("区间分割法近似根$x^*$的收敛曲线（前15次迭代）", fontdict={"fontsize": 18})

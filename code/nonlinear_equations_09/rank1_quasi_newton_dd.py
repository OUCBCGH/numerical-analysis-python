...
class Rank1QuasiNewton(NonLinearEquationsUtils):
    ...
    继承NonLinearEquationsUtils, 默认采用离散差商矩阵作为初始的A0
    """

    def __init__(self, nlin_fxs, x0, max_iter=200, eps=1e-10, method="broyden",
                 is_plt=False, diff_mat_h=0.01, A0="diff_mat"):
        NonLinearEquationsUtils.__init__(self, nlin_fxs, x0, max_iter,
                                         eps, is_plt)  # 父类初始化
    ...
    def fit_nlin_roots(self):
        """
        对称秩1迭代法求解非线性方程组的解，核心算法
        :return:
        """
        iter_, sol_tol, x_n = 0, np.infty, np.copy(self.x0)  # 初始化
        if self.A0.lower() == "diff_mat":  # 计算差商离散矩阵作为初始修正矩阵
    ...
    def _solve_broyden_(self, Ak, sol_tol, iter_, x_n):
        ...
        while np.abs(sol_tol) > self.eps and iter_ < self.max_iter:
            ...
            sol_xb = self.nlin_Fxs(x_b)  # F(xk)，构成一个向量
    ...
    def _solve_inv_broyden_(self, Ak, sol_tol, iter_, x_n):
        ...
        while np.abs(sol_tol) > self.eps and iter_ < self.max_iter:
            x_b = np.copy(x_n)  # 更新解向量
            sol_xb = self.nlin_Fxs(x_b)  # 解向量x_k的方程组的值，向量

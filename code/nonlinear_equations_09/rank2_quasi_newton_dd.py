...
"""
@file:rank1_quasi_newton_dd.py.py
...
class Rank2QuasiNewton(NonLinearEquationsUtils):
    """
    对称秩算法求解非线性方程组的解，包括对称秩2算法中的DFP、BFS、BFGS和逆BFGS算法.
    继承NonLinearEquationsUtils, 默认采用离散差商矩阵作为初始的H0
    """

    def __init__(self, nonlinear_equations, x0, max_iter=200, eps=1e-10, method="dfp",
                 is_plt=False, diff_mat_h=0.01, A0="diff_mat"):
        NonLinearEquationsUtils.__init__(self, nonlinear_equations, x0, max_iter, eps, is_plt)
        self.n = len(x0)  # 解的个数
        self.method = method  # 秩2四种算法
        self.diff_mat_h = diff_mat_h  # 求解离散差商矩阵的步长
        self.A0 = A0  # 初始修正矩阵
        self.fxs_precision = None  # 最终解向量针对每个方程的精度

    def fit_nlin_roots(self):
        """
        秩2迭代法求解非线性方程组的解，采用离散差商矩阵
        :return:
        """
        iter_, sol_tol, x_n = 0, np.infty, np.copy(self.x0)  # 初始化
        if self.A0.lower() == "diff_mat":  # 计算差商离散矩阵作为初始矩阵，不再求解F的雅可比矩阵
            Ak = np.zeros((self.n, self.n))
            for i in range(self.n):
                x_d, h = np.copy(self.x0), self.diff_mat_h
                x_d[i] += h
                Ak[:, i] = ((self.nlin_Fxs(x_d) - self.nlin_Fxs(self.x0)) / h).flatten()
        else:
            Ak = np.eye(self.n)  # 初始化为单位矩阵
    ...
    def _solve_DFP_rank2_(self, Hk, sol_tol, iter_, x_n):
        ...
        while np.abs(sol_tol) > self.eps and iter_ < self.max_iter:
            ...
            sol_xb = self.nlin_Fxs(x_b)  # 上一次数值解的函数值向量F(x_k)
            x_n = x_b - np.dot(Hk, sol_xb)  # 计算新的数值解向量x_{k+1}
            sol_xn = self.nlin_Fxs(x_n)  # 新的数值解的函数值向量F(x_{k+1})
    ...
    def _solve_inv_BFGS_rank2_(self, Hk, sol_tol, iter_, x_n):
        ...
        while np.abs(sol_tol) > self.eps and iter_ < self.max_iter:
            ...
            term1 = np.dot(s_k - np.dot(Hk, y_k), s_k.T) + np.dot(s_k, (s_k - np.dot(Hk, y_k)).T)

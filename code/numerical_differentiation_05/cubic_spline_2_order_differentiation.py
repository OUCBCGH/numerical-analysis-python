...
"""
@file_name: cubic_spline_2_order_differentiation.py
@time: 2021-11-25
...
from numerical_differentiation_05.discrete_data_cubic_spline_differetiation \
    import DiscreteDataCubicSplineDifferential  # 三次样条函数求解一阶数值微分类


class CubicSpline2OrderDifferentiation(DiscreteDataCubicSplineDifferential):
    """
    三次样条插值求解二阶数值微分，继承DiscreteDataCubicSplineDifferetiation
    """

    def cal_diff(self, x0):
        """
        三次样条方法求解二阶数值微分核心算法，重写父类实例方法
        :return:
        """
        ...
        # M = self._cal_natural_spline_()  # 求解样条系数， 自然样条条件
        for i in range(len(x0)):  # 逐个求解给定值的微分
            ...
            # 分段三次样条插值二阶导函数公式
            self.diff_value[i] = (self.x[idx + 1] - x0[i]) / h * M[idx] + \
                                 (x0[i] - self.x[idx]) / h * M[idx + 1]

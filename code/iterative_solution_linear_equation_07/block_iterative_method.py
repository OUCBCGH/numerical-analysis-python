...
class BlockIterative(IterativeLinearEquationsUtils):
    ...
    def __init__(self, A, b, x0, block, eps=1e-8, max_iter=1000, omega=1.5,
        ...
        IterativeLinearEquationsUtils.__init__(self, A, b, x0, eps, max_iter,
                                               is_out_info)
        self.block = np.asarray(block, dtype=np.int64)  # 分块向量
    ...
    def fit_solve(self):
        ...
        # 分析块索引，即每块的起始索引
        n_b = len(self.block)
        block_start_idx = np.zeros(n_b, dtype=np.int64)
        ...
        for i in range(n_b):
            ...
            D[s_idx:e_idx, s_idx:e_idx] = self.A[s_idx:e_idx, s_idx:e_idx]  # 分块对角矩阵A_{ii}
            # A对角分块矩阵的逆矩阵
            inv_D[s_idx:e_idx, s_idx:e_idx] = \
                np.linalg.inv(self.A[s_idx:e_idx, s_idx:e_idx])
        # 求系数矩阵A的下三角分块阵和上三角分块阵
        b_low_mat, b_up_mat = -np.tril(self.A - D), -np.triu(self.A - D)
        if self._is_convergence_(D, inv_D, b_low_mat, b_up_mat):
            if self.method.lower() == "jacobi":
                self.x = self._solve_block_jacobi_(D, inv_D)
            elif self.method.lower() == "g-s":
                self.x = \
                    self._solve_block_gauss_seidel_(D, b_low_mat, b_up_mat)
            elif self.method.lower() == "sor":
                self.x = self._solve_block_sor_(D, b_low_mat, b_up_mat)
            else:
    ...
    def _solve_block_gauss_seidel_(self, D, block_low_mat, block_up_mat):
        ...
        for iteration in range(self.max_iter):
            ...
            # 块G-S矩阵形式迭代公式
            x_next = np.dot(np.dot(inv_DL, block_up_mat), x_before) + \
                     np.dot(inv_DL, self.b)
            tol = np.linalg.norm(x_next - x_before)
            self.precision.append(np.linalg.norm(self.b - np.dot(self.A, x_next)))  # 每次迭代的误差
            if self.precision[-1] <= self.eps or tol <= self.eps:  # 满足精度要求，迭代终止
                break
    ...
    def _solve_block_sor_(self, D, block_low_mat, block_up_mat):
        ...
        for iteration in range(self.max_iter):
            x_before = np.copy(x_next)  # 迭代序列更新
            # 块超松弛矩阵形式迭代公式
            x_next = np.dot(np.dot(inv_DL, omega_ * D +
                                   self.omega * block_up_mat), x_before) + \
                     self.omega * np.dot(inv_DL, self.b)
            tol = np.linalg.norm(x_next - x_before)
            self.precision.append(np.linalg.norm(self.b - np.dot(self.A, x_next)))  # 每次迭代的误差
            if self.precision[-1] <= self.eps or tol <= self.eps:  # 满足精度要求，迭代终止
                break

...
"""
@author:Lenovo
@file:test_cubic_corner.py
@time:2021/08/30
"""
import numpy as np
import sympy
from interpolation_02.cubic_spline_corner_interpolation import CubicSplineCornerInterpolation
...
# print(dy[0], dy[-1])
# print(d2y[0], d2y[-1])
# x0 = np.array([np.pi / 2, 2.6, 4.0, 4.8, 2 * np.pi])

# plt.figure(figsize=(14, 5))
# boundary_cond = ["natural", "periodic"]
# for i, bc in enumerate(boundary_cond):
#     csi = CubicSplineCornerInterpolation(x, y, dy=dy, d2y=d2y, boundary_cond=bc)
#     csi.fit_interp()
...
np.random.seed(100)
t = sympy.Symbol("t")
# x = np.linspace(a, b, 20, endpoint=True)  # 等距
x = a + (b - a) * np.random.rand(20)  # 非等距
x = np.sort(x)  # 排序
...
plt.figure(figsize=(14, 5))
plt.subplot(121)
csi = CubicSplineCornerInterpolation(x, y, dy=dy, d2y=d2y, boundary_cond="complete")
csi.fit_interp()
y0 = csi.predict_x0(x0)
...
plt.title("三次埃尔米特插值不同边界条件下的绝对值误差", fontdict={"fontsize": 18})
plt.legend(frameon=False, fontsize=16)
plt.tick_params(labelsize=18)  # 刻度字体大小18
plt.grid(ls=":")
# plt.ylim([5e-6, 1.5e+01])  # 非周期函数
...
y = np.array([12, 9, 9, 10, 18, 24, 28, 27, 25, 20, 18, 15, 13])  # 温度值
x0 = [13, 17.4, 22.8]

plt.figure(figsize=(14, 5))
plt.subplot(121)
csi = CubicSplineCornerInterpolation(x, y, boundary_cond="natural")
csi.fit_interp()
y0 = csi.predict_x0(x0)

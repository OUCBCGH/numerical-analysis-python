...
fun = -1 * (x[0] ** 2 - 2 * x[0]) * sympy.exp(-x[0] ** 2 - x[1] ** 2 - x[0] * x[1])  # 目标函数极大值
# fun = (x[0] ** 2 - 2 * x[0]) * sympy.exp(-x[0] ** 2 - x[1] ** 2 - x[0] * x[1])  # 目标函极小值
x0, eps = [-1, 0], 1e-16  # 极大值
# x0, eps = [0, 0], 1e-16  # 极小值

# 例6示例
# fun = (x[0] - x[1]) / (x[0] ** 2 + x[1] ** 2 + 2)
# x0, eps = [-0.3, 0.2], 1e-16


dfp = DFPQuasiNewtonOptimization(fun, x0, eps=eps, is_minimum=False)  # 修改is_minimum变换极小极大值
sol = dfp.fit_optimize()
print(dfp.local_extremum)
print("最优解：%.15f, %.15f， 最优值：%.15f" % (sol[0], sol[1], sol[2]))
dfp.plt_optimization([-3, 3], [-3, 3])

...
from iterative_solution_linear_equation_07.pre_conjugate_gradient \
    import PreConjugateGradient  # 第7章预处理共轭梯度算法
...
class PDEPoissonEquationTriBMatrix:
    ...
    def __init__(self, fxy_fun, f_ux0, f_uxb, f_u0y, f_uay,
        ...
        self.x_span, self.y_span = np.asarray(x_span, np.float64), np.asarray(y_span, np.float64)
    ...
    def solve_pde(self):
        ...
        # 构造右端向量，构成右端fi
        # (1) pde右端方程，内部节点(n-1)*(m-1)
        fi = self.fxy_fun(xm[1: -1, 1: -1], ym[1: -1, 1: -1])
        # (2) 第一行单独处理，φ(x0,yj)
        fi[0, :] = fi[0, :] + self.u_xy[0, 1:-1] / self.h_x ** 2
        # (3) # 最后一行单独处理，φ(xm,yj)
        fi[-1, :] = fi[-1, :] + self.u_xy[-1, 1:-1] / self.h_x ** 2
        fi = fi.T.flatten()  # (4) 展平成向量，默认按行
        ...
        pcg = PreConjugateGradient(difference_matrix.toarray() / self.n_x ** 2,
                                   fi / self.n_x ** 2, np.zeros(len(fi)),

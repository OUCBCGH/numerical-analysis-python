    ...
    def _solve_jacobi_(self):
        ...
        for iteration in range(self.max_iter):
            ...
            for i in range(self.n):
                sum_j = np.dot(self.A[i, :i], x_before[:i]) + np.dot(self.A[i, i + 1:], x_before[i + 1:])
    ...
    def _solve_gauss_seidel_(self):
        ...
        for iteration in range(self.max_iter):
            for j in range(self.n):
                # G-S不同于Jacobi迭代法的地方，即np.dot(self.A[j, :j], x_iter[:j])每次采用新迭代值
                sum_g = np.dot(self.A[j, :j], x_iter[:j]) + np.dot(self.A[j, j + 1:], x_iter[j + 1:])

...
class IterativeLinearEquationsUtils:
    ...
    def __init__(self, A, b, x0, eps=1e-8, max_iter=200, is_out_info=False):
        self.A = np.asarray(A, dtype=np.float)  # 系数矩阵
        self.b = np.asarray(b, dtype=np.float)  # 右端向量
        self.x0 = np.array(x0, dtype=np.float)  # 迭代初值
    ...
    def _plt_convergence_precision(self, is_show=True, method="", style="-"):
        """
        ...
        plt.semilogy(iter_num, self.precision, "%s" % style, lw=2,
                     label="$\epsilon=%.3e, \ k=%d$" % (self.precision[-1], iter_num[-1])) # 对数坐标
        plt.xlabel("$Iterations(k)$", fontdict={"fontsize": 18})
        plt.ylabel("$Precision(\epsilon)$", fontdict={"fontsize": 18})
        plt.title("$%s$的$\epsilon=\Vert b - Ax^* \Vert _2$的收敛曲线" % method, fontdict={"fontsize": 18})

...
equation = lambda x: x ** 4 - 8 * x ** 3 + 3 * x ** 2 + 10 * x - 1  # 待求方程

options = {"eps": 1e-16, "display": "display", "pltFuns": False}  # 参数设置，某些参数可不设置
funEvals = ["regula", "dichotomy", "golden"]
linestyle = ["-", "--", "-."]
roots, eps = [], []
plt.figure(figsize=(14, 5))
for i, funEval in enumerate(funEvals):
    ism = IntervalSegmentation_Root(equation, [-0.1, 0.2], eps=1e-16, display="display", funEval=funEval)  # 实例化对象
    ism.fit_root()  # 求近似根
...
plt.plot(np.arange(1, 11), roots[1][:10], "--o", lw=2, label="$%s: \ x^*=%.15f$" % (funEvals[1], roots[1][-1]))
plt.plot(np.arange(1, 11), roots[2][:10], "-.*", lw=2, label="$%s: \ x^*=%.15f$" % (funEvals[2], roots[2][-1]))
plt.xlabel("$Iterations(k)$", fontdict={"fontsize": 18})
plt.ylabel("$x^*$", fontdict={"fontsize": 18})
plt.title("区间分割法近似根$x^*$的收敛曲线（前10次迭代）", fontdict={"fontsize": 18})
plt.legend(frameon=False, fontsize=16)
plt.tick_params(labelsize=16)  # 刻度字体大小16
plt.grid(ls=":")
plt.subplot(122)
plt.plot(np.arange(1, len(eps[0]) + 1), eps[0], "-s", lw=2, label="$%s: \ k=%d$" % (funEvals[0], len(eps[0])))
plt.plot(np.arange(1, 11), eps[1][:10], "--o", lw=2, label="$%s: \ k=%d$" % (funEvals[1], len(eps[1])))
plt.plot(np.arange(1, 11), eps[2][:10], "-.*", lw=2, label="$%s: \ k=%d$" % (funEvals[2], len(eps[2])))
plt.xlabel("$Iterations(k)$", fontdict={"fontsize": 18})
